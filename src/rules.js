...
export class Rules {
    ...
    **/
    check_with_ko_and_suicide(pt, color) {
        if (!this.check(pt, color))
            return false;
        ...
        let ser_str         = this.board.get_serialized_state_str();
        let is_suicide_move = this.board.isFree(pt);

        this.board.revert(1);
        if (is_suicide_move)
            return false;

        if (this.board.check_state_already_exists(ser_str))
            return false;
    ...
    **/
    ...
    doCapture(pt, color) {
        ...
        //d// console.log("{rules} doCapture", [pt, color, this.pendingCaptures.length]);
